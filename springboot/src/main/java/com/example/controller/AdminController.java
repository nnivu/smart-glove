...
import com.example.entity.Admin; // 导入管理员实体类，用于数据封装和传输
import com.example.service.AdminService; // 导入管理员业务逻辑服务类，控制器通过调用服务层实现业务功能
import jakarta.annotation.Resource; // 导入依赖注入注解，用于自动注入Service实例
import org.springframework.web.bind.annotation.*; // 导入Spring MVC相关注解，实现RESTful API

import java.util.List;
import com.example.common.Result; // 导入统一响应结果封装类，保证接口返回格式一致性

@RestController // 标识该类为REST风格控制器，所有方法返回数据默认会被转换为JSON格式
@RequestMapping("/admin") // 定义基础请求路径，当前控制器所有接口的URL前缀为 /admin
public class AdminController {

    // 注入AdminService接口的实现类对象，由Spring容器自动管理和注入
     ...
     * @param name 管理员名称（通过URL查询参数传递，如：/admin?name=admin1）
     ...
     */
    @GetMapping("") // 处理HTTP GET请求，完整访问路径为 /admin
    public Result getAdminByName(String name) {
     ...
     * @param admin 管理员实体对象，通过请求体（RequestBody）以JSON格式传递
     *              包含管理员的各项属性（如用户名、密码、权限等）
     * @return Result对象，包含操作结果：
     *         - 成功：状态码200，无具体数据（或包含新创建管理员的ID）
     *         - 失败：状态码非200，message字段包含错误信息（如用户名已存在）
     */
    @PostMapping("/add") // 处理HTTP POST请求，完整访问路径为 /admin/add
    public Result add(@RequestBody Admin admin) { // @RequestBody注解自动将JSON请求体转换为Admin对象
     ...
     * @param admin 包含更新信息的管理员实体对象，通过请求体以JSON格式传递
     *              必须包含管理员ID（用于定位要更新的记录）和需要修改的字段
     * @return Result对象，包含操作结果：
     *         - 成功：状态码200
     *         - 失败：状态码非200（如管理员不存在、数据验证失败）
     */
    @PutMapping("/update") // 处理HTTP PUT请求，完整访问路径为 /admin/update
    public Result update(@RequestBody Admin admin) { // 接收JSON格式的更新数据
     ...
     * @param id 管理员ID，通过URL路径参数传递（如：/admin/delete/1001）
     * @return Result对象，包含操作结果：
     *         - 成功：状态码200
     ...
     */
    @DeleteMapping("/deleteBatch") // 处理HTTP DELETE请求，完整路径为 /admin/deleteBatch
    public Result deleteBatch(@RequestBody List<Admin> list) { // 接收JSON数组并转换为List<Admin>
     ...
     * @return Result对象，包含查询结果：
     *         - 成功：状态码200，data字段为List<Admin>集合
     *         - 失败：状态码非200
     * 注意：当数据量较大时，不建议使用此接口，应采用分页查询
     */
    @GetMapping("/selectAll") // 处理HTTP GET请求，完整路径为 /admin/selectAll
